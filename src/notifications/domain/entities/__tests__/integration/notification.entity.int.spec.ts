...
describe('NotificationEntity integration tests', () => {
  ...
  describe('Constructor', () => {
    ...
    it('Should throw error when destinatario is invalid', () => {
      expect(
        () => new NotificationEntity({ ...props, destinatario: '' })
      ).toThrowError('Entity Validation Error');
      expect(
        () => new NotificationEntity({ ...props, destinatario: null })
      ).toThrowError('Entity Validation Error');
      expect(
        () => new NotificationEntity({ ...props, destinatario: undefined })
      ).toThrowError('Entity Validation Error');
      expect(
        () => new NotificationEntity({ ...props, destinatario: 123 as any })
      ).toThrowError('Entity Validation Error');
    });

    it('Should throw error when titulo is invalid', () => {
      expect(
        () => new NotificationEntity({ ...props, titulo: '' })
      ).toThrowError('Entity Validation Error');
      expect(
        () => new NotificationEntity({ ...props, titulo: 123 as any })
      ).toThrowError('Entity Validation Error');
      expect(
        () => new NotificationEntity({ ...props, titulo: 'a'.repeat(256) })
      ).toThrowError('Entity Validation Error');
    });

    it('Should throw error when mensagem is invalid', () => {
      expect(
        () => new NotificationEntity({ ...props, mensagem: '' })
      ).toThrowError('Entity Validation Error');
      expect(
        () => new NotificationEntity({ ...props, mensagem: 123 as any })
      ).toThrowError('Entity Validation Error');
    });
    ...
    it('Should throw error when enviadoEm is invalid', () => {
      expect(
        () =>
          new NotificationEntity({ ...props, enviadoEm: 'invalid-date' as any })
      ).toThrowError('Entity Validation Error');
      expect(
        () => new NotificationEntity({ ...props, enviadoEm: 123 as any })
      ).toThrowError('Entity Validation Error');
    });
  });
  ...
  describe('Setters', () => {
    it('Should throw error when update with invalid titulo', () => {
      let newTitulo = '';
      expect(() => sut.updateTitulo(newTitulo)).toThrowError(
    ...
    it('Should throw error when update with invalid mensagem', () => {
      let newMensagem = '';
      expect(() => sut.updateMensagem(newMensagem)).toThrowError(
        'Entity Validation Error'
      );
